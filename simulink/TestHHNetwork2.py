...
if __name__ == '__main__':
    ...
    manager = Manager()
    comp = [[0 for i in range(10)] for j in range(3)]
    syn = [[0 for i in range(10)] for j in range(3)]
    para1 = []
    para2 = []
    for i in range(10):
        para1.append(0.015)
    ...
    for i in range(3):
        for j in range(10):
            if (i == 0):
                ...
                if (a != 0):
                    comp[i][j] = HHNeuron(manager, 'comp' + str(i) + str(j), 10)
                else:
                    comp[i][j] = HHNeuron(manager, 'comp' + str(i) + str(j), 10)
                syn[i][j] = synapseNeuron(manager, 'syn' + str(i) + str(j), para1)
            else:
                if (a != 0):
                    comp[i][j] = HHNeuron(manager, 'comp' + str(i) + str(j), 0)
                else:
                    comp[i][j] = HHNeuron(manager, 'comp' + str(i) + str(j), 0)
